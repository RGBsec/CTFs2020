...
def SaveAsZip(zipname, filename, filedata):
    ...
    def LocalFileHeader(name, size):
        ...
        extraLength = 0
        return b"PK\x03\x04" + struct.pack("<3H4LHH", neededVersion, flags, method, timestamp, crc32, compressedSize,
                                           originalSize, nameLength, extraLength) + utf8name
    ...
    def DirEntry(name, size):
        ...
        osAttrs = 0
        dataOfs = 0
        return b"PK\x01\x02" + struct.pack("<4H4L5HLL", createVersion, neededVersion, flags, method, timestamp, crc32,
                                           compressedSize, originalSize, nameLength, extraLength, commentLength,
                                           diskNrStart, zipAttrs, osAttrs, dataOfs) + utf8name
    ...
    def EndofZip(dirSize, dirOfs):
        ...
        totalEntries = 1
        commentLength = 0
        return b"PK\x05\x06" + struct.pack("<4HLLH", thisDiskNr, startDiskNr, thisEntries, totalEntries, dirSize,
                                           dirOfs, commentLength)
...
def password_cracker(data, args):
    ...
    for password in pwgen:
        ...
        count += 1
        if count % 1000 == 0:
            print("%8d passwords tried, %d passwords per second" % (count, count / (time.time() - t0)))
...
def main():
    ...
    parser.add_argument('--writezip', '-w', action='store_true',
                        help='Save zip encrypted data to a .zip file for cracking')
