...
def reconstitute_secret(keys, matrix):
    ...
    print("subshares:", subshares)
    key = (int(sum([i * j for i, j in zip(
        [i * pow(det, -1, P) for i in [int(round(h)) for h in [det * inv_float[0][i] for i in range(k)]]],
        subshares)])) % P).to_bytes(32, byteorder='little')
    # key = (int(sum([i * j for i, j in zip(
    #     [int(round(det * inv_float[0][i])) * pow(det, -1, P) for i in range(k)],
    #     subshares)])) % P).to_bytes(32, byteorder='little')
    print("key:",key)
    print("keynum:", int.from_bytes(key, 'little'))
    # print(pow(det, -1, P))
    for row in range(5):
        print([(i * pow(det, -1, P)) % P for i in [int(round(h)) for h in [det * inv_float[row][i] for i in range(k)]]])
    # print([(i * pow(det, -1, P)) for i in [int(round(h)) for h in [det * inv_float[0][i] for i in range(k)]]])
    print(det)
...
def share_actual_flag():
    ...
    print("DEBUG secret shares:", shares[:2])
...
def login():
    ...
    print(banner)
    print('\n'.join(textwrap.wrap(
        "OOO has finally solved the flag sharing problem by making it quick and easy for aspiring cheaters to share flags by utilizing a secure and exciting secret sharing scheme! OOO reserves the right to withhold flag shares where deemed appropriate.",
        width=80)))
    print()
...
def my_gensafeprime_generate(bits=256):
    # from sympy import randprime, isprime
    # p = randprime(0, 1 << (bits - 1))
    # while not isprime(2 * p + 1):
    #     p = randprime(0, 1 << (bits - 1))
    # return 2 * p + 1
    return 95820804521871446624646154398560990164494336030962272285033480112778980081147
...
if not os.path.exists(os.path.join(os.path.dirname(__file__), "prime.ooo")):
    ...
    with open("prime.ooo", 'w') as _f:
        ...
        _f.write(str(my_gensafeprime_generate(256)))
