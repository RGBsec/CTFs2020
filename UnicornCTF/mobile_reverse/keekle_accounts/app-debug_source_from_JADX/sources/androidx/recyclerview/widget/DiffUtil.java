...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new Comparator<Snake>() {
        public int compare(Snake o1, Snake o2) {
            int cmpX = o1.f38x - o2.f38x;
            return cmpX == 0 ? o1.f39y - o2.f39y : cmpX;
        }
    };
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (firstSnake == null || firstSnake.f38x != 0 || firstSnake.f39y != 0) {
        ...
        private void findMatchingItems() {
            ...
            for (int i = this.mSnakes.size() - 1; i >= 0; i--) {
                ...
                int endX = snake.f38x + snake.size;
                int endY = snake.f39y + snake.size;
                ...
                for (int j = 0; j < snake.size; j++) {
                    int oldItemPos = snake.f38x + j;
                    int newItemPos = snake.f39y + j;
        ...
        public void dispatchUpdatesTo(ListUpdateCallback updateCallback) {
            ...
            for (int snakeIndex = this.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                int endX = snake.f38x + snakeSize2;
                int endY2 = snake.f39y + snakeSize2;
                if (endX < posOld2) {
                    endY = endY2;
                ...
                for (int i2 = snakeSize - 1; i2 >= 0; i2--) {
                    if ((this.mOldItemStatuses[snake.f38x + i2] & 31) == 2) {
                        batchingCallback.onChanged(snake.f38x + i2, 1, this.mCallback.getChangePayload(snake.f38x + i2, snake.f39y + i2));
                    }
                }
                posOld2 = snake.f38x;
                posNew = snake.f39y;
    ...
    public static DiffResult calculateDiff(Callback cb, boolean detectMoves) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (snake != null) {
                ...
                snake.f38x += range.oldListStart;
                snake.f39y += range.newListStart;
                Range left = arrayList2.isEmpty() ? new Range() : (Range) arrayList2.remove(arrayList2.size() - 1);
                left.oldListStart = range.oldListStart;
                left.newListStart = range.newListStart;
                if (snake.reverse) {
                    left.oldListEnd = snake.f38x;
                    left.newListEnd = snake.f39y;
                } else if (snake.removal) {
                    left.oldListEnd = snake.f38x - 1;
                    left.newListEnd = snake.f39y;
                } else {
                    left.oldListEnd = snake.f38x;
                    left.newListEnd = snake.f39y - 1;
                }
                stack.add(left);
                Range right = range;
                if (!snake.reverse) {
                    right.oldListStart = snake.f38x + snake.size;
                    right.newListStart = snake.f39y + snake.size;
                } else if (snake.removal) {
                    right.oldListStart = snake.f38x + snake.size + 1;
                    right.newListStart = snake.f39y + snake.size;
                } else {
                    right.oldListStart = snake.f38x + snake.size;
                    right.newListStart = snake.f39y + snake.size + 1;
                }
                stack.add(right);
            } else {
            ...
            for (int d = 0; d <= dLimit; d++) {
                ...
                while (k <= d) {
                    ...
                    if (!checkInFwd || k < (delta - d) + 1 || k > (delta + d) - 1 || iArr[kOffset + k] < iArr2[kOffset + k]) {
                        ...
                        outSnake.f38x = iArr2[kOffset + k];
                        outSnake.f39y = outSnake.f38x - k;
                ...
                while (k2 <= d) {
                    ...
                    if (checkInFwd || k2 + delta < (-d) || k2 + delta > d || iArr[kOffset + backwardK] < iArr2[kOffset + backwardK]) {
                        ...
                        outSnake2.f38x = iArr2[kOffset + backwardK];
                        outSnake2.f39y = outSnake2.f38x - backwardK;
