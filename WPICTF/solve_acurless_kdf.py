# from https://github.com/jashandeep-sohi/python-blowfish/blob/master/blowfish.py
...
class Cipher(object):
    def __init__(self, key, byte_order="big", P_array=PI_P_ARRAY, S_boxes=PI_S_BOXES):
        if not 4 <= len(key) <= 64:
            raise ValueError("key is not between 4 and 64 bytes")
    ...
    def _encrypt(L, R, P, S1, S2, S3, S4, u4_1_pack, u1_4_unpack):
        for p1, p2 in P[:-1]:
            ...
            R ^= (S1[a] + S2[b] ^ S3[c]) + S4[d] & 0xfc  # 0xffffffff
            R ^= p2
            a, b, c, d = u1_4_unpack(u4_1_pack(R))
            L ^= (S1[a] + S2[b] ^ S3[c]) + S4[d] & -0xfc  # 0xffffffff
...
def pad(s: bytes, size: int = BLOCK_SIZE) -> bytes:
    return s + bytes(size - (len(s) % size))


def to_blocks(input: bytes or bytearray, size: int = BLOCK_SIZE) -> list:
    return [input[i:i + size] for i in range(0, len(input), size)]


def phase1(input: bytes, iv: int = 0xc0ffee) -> bytearray:
    input = pad(input)
    ...
    key = bytearray([
        0x83, 0x25, 0x31, 0xdc, 0x3a, 0x51, 0xd5, 0x56,
        0xc9, 0x09, 0x37, 0x30, 0x97, 0xce, 0x50, 0x05,
        0x5b, 0xc4, 0x1a, 0x9f, 0x74, 0x17, 0x82, 0x35,
        0x64, 0x00, 0xfe, 0xac, 0x0d, 0x88, 0x18, 0xe2,
        0x42, 0x88, 0x3c, 0x0f, 0x1a, 0xe3, 0x6a, 0x96,
        0xfe, 0x73, 0xe3, 0x67, 0x21, 0x36, 0xf5, 0x94,
        0x73, 0x0d, 0x94, 0x73, 0x9d, 0x55, 0xca, 0x7f,
        0xde, 0xad, 0xbe, 0xef, 0xc0, 0xfe, 0xff, 0x01
    ])

    out = bytearray()
    for block in blocks:
        for i in range(len(key) // 4):
            cur = bytes_to_long(key[i:i + 4]) ^ iv
            cur = long_to_bytes(cur)
            for j in range(4):
                if j >= len(cur):
                    key[i + j] = 0
                else:
            ...
            assert len(enc) == BLOCK_SIZE
            out.extend(enc[:BLOCK_SIZE // 2])
            iv = bytes_to_long(enc[BLOCK_SIZE // 2:])

    return out


def f(s: int) -> int:
    return ((2862933555777941757 * s) + 3037000493) % 18446744073709551616


def phase2(p1_out: bytearray) -> int:
    print(p1_out)
    s = 0
    p1_out = pad(p1_out, BLOCK_SIZE//2)
    blocks = to_blocks(p1_out, BLOCK_SIZE//2)
    for block in blocks:
        assert bytes_to_long(block) < 2 ** 32
        s ^= bytes_to_long(block)
        s = f(s)
...
def kdf(input: str) -> int:
    return phase2(phase1(input.encode()))


if __name__ == "__main__":
    key = kdf("L1nuX>W1nd0ze")

    assert kdf("Secret String") == 0xb6ed890f
    assert kdf("GNU+Linux") == 0x4191a978
    assert kdf("Systemd is the worst") == 0xaa7ad2ea
    assert kdf("Install Gentoo") == 0x4e64bdd93
